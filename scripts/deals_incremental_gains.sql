...
CREATE TEMP TABLE daily_coop_results AS (
    SELECT 
        ...
        SUM(coop_amount) as daily_coop_amount,
        MAX(coop_amount_currency) as coop_amount_currency  -- assuming this is constant per group
    FROM andes.rs_coop_ddl.coop_csi_calculation_results
    WHERE is_valid = 'Y'
        AND TO_DATE(order_datetime,'YYYY-MM-DD') 
    ...
    GROUP BY 
        ...
        gl_product_group_id,
        agreement_id,
        TO_DATE(order_datetime,'YYYY-MM-DD')
);
